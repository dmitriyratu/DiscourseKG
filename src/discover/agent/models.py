...
class DateSource(str, Enum):
    ...
    def _source(name: str, description: str, weight: int) -> tuple[str, str, int]:
        """Helper to create date source tuple with named parameters."""
        return (name, description, weight)

    datetime_attr = _source(name="datetime_attr", description="<time datetime=\"...\">", weight=7)
    schema_org = _source(name="schema_org", description="schema.org datePublished", weight=5)
    url_path = _source(name="url_path", description="date segment in the URL path (e.g. .../january-28-2026/, .../2026/01/28/)", weight=3)
    near_title = _source(name="near_title", description="date in the article title or on the line immediately above/below the title", weight=2)
    metadata = _source(name="metadata", description="other page metadata", weight=1)
